...
public class Iwk_videoDaoImpl {
	/**
	 * 视频数据类 方法： 1.删除（id,对象），返回boolean； 2.查询（id,对象），返回object；
	 * 3.保存（对象），返回boolean； 4.修改（对象），返回boolean； 5.取等条件查询（对象类，字段，值），返回集合；
	 * 6.获取所有记录，返回集合； 7.集合内对象类别转换;
	 */
	...
	public boolean delectById(int id) {
		...
		video = queryById(id);
		//删除视频
		FileDeleteTool.deleteFile(video.getSrcs());
		//删除视频
		FileDeleteTool.deleteFile(video.getPic());
	...
	public List<Video> queryByWhere(String keys, Object values) {
		...
		try {
			...
			System.out
					.println("Iwk_VideoDaoImpl的query(String keys,Object values)报错："
							+ e.getMessage());
			return null;
		}
	}
	...
	public List<Video> Object2Other(List<Object> list) {
		...
		try {
			...
			System.out
					.println("Iwk_VideoDaoImpl的Object2Other(List<Object> list)报错："
							+ e.getMessage());
			return null;
		}
	...
	public List<Video> queryOrderDesc(String keys, int maxResults) {
		...
		try {
			...
			System.out.println("Iwk_VideoDaoImpl的queryOrderDesc报错："
					+ e.getMessage());
			return null;
		}
	...
	public List<Video> queryOrderAsc(String keys, int maxResults) {
		...
		try {
			...
			System.out.println("Iwk_VideoDaoImpl的queryOrderAsc报错："
					+ e.getMessage());
			return null;
		}
	...
	// 11.分页查询
	public List<Video> queryPageVideo(int curPage, int max, String types,
			String values) {
		video = new Video();
		try {
			return Object2Other(iwkDao
					.query(video, curPage, max, types, values));
		} catch (Exception e) {
			System.out.println("Iwk_VideoDaoImpl的queryOrderAsc报错："
					+ e.getMessage());
			return null;
	...
	// 11.查询单人的视频
	public List<Video> queryPageVideoByObjid(String key, String values) {
		video = new Video();
		try {
			return Object2Other(iwkDao.query(video, key, values));
		} catch (Exception e) {
			System.out.println("Iwk_VideoDaoImpl的queryOrderAsc报错："
					+ e.getMessage());
			return null;
	...
	public double queryRecordNum(String types, String values) {
		...
		try {
			...
			System.out.println("Iwk_VideoDaoImpl的queryRecordNum报错："
					+ e.getMessage());
